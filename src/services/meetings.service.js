...
export class MeetingsService {
  ...
  findAndGetMeeting = async (type, userId) => {
    ...
    // type을 가져올 시
    if (type === 'GROUP') {
      maxMeetingCapacity = 8; // 테스트를 위한 2 설정 maxMeetingCapacity = 6
    } else if (type === 'COUPLE') {
    ...
    const meetingAndUser = await this.meetingsRepository.existMeetingsAndUsers(
      type,
    );
    ...
    const meetingsIsFull = meetingAndUser.every(
      (meeting) => meeting.Members.length >= maxMeetingCapacity,
    );
    ...
    if (!meetingAndUser || meetingsIsFull) {
      ...
      meeting = meetingAndUser.find((user) => {
        const maleCount = user.Members.filter(
          (member) => member.Users.gender === 'MALE',
        ).length;
        console.log('maleCount 확인 ', maleCount);

        const femaleCount = user.Members.filter(
          (member) => member.Users.gender === 'FEMALE',
        ).length;
        console.log('femaleCount 확인 ', femaleCount);

        const isMaleFull = userGender === 'MALE' && maleCount === maxGender;
        const isFemaleFull =
          userGender === 'FEMALE' && femaleCount === maxGender;
  ...
  createQuestion = async () => {
    ...
    const createQuestion = await this.meetingsRepository.createQuestion(
      question,
    );
