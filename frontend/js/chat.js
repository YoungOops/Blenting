// socket.io 클라이언트 라이브러리를 사용하여 서버에 연결을 시도합니다.
const socket = io();
...
// 엔드포인트 아니고 이벤트에요, 소켓은 이벤트 네임이 같을 때 통신 가능합니다
// 서버로부터 'entry' [이벤트]* 를 받으면, 새로운 사용자가 입장했음을 알리는 메시지를 화면에 표시합니다.
socket.on('entry', (data) => {
  console.log(data); // 서버에서 받은 데이터를 콘솔에 로그합니다.
  const item = document.createElement('li'); // 새로운 'li' 요소를 생성합니다.
  item.textContent = data.id + '님이 입장 하였습니다.'; // 'li' 요소에 텍스트를 추가합니다.
